...
Also, the data is pushed to DB after checking imei
...
def writeToDB(data, client):
    ...
    if len(user.last50kData) > 50000:
        user.last50kData.pop() 

    user.save()
    print("\nDATA PUBLISHED ==>", data[client])

    date_str = insertable_data['time'].split(' ')[0].strip()  # Extract date
    year, month, day = date_str.split('/')  # Assuming the format is 'YYYY/MM/DD'
    ...
    print(f"Data logged to {file_path} for {data[client]['imei']}!")
...
def LOGGER(event, filename, ip, client, type, data):
    ...
    with open(os.path.join('', filename), 'a+') as log:
        if (event == 'info'):
...
def answer_gps(client, query):
    ...
    data[client]['time']= datetime.now().strftime('%Y/%m/%d %H:%M:%S')
